...
function Reservations() {
	...
	useEffect(() => {
		...
		console.log(
			'current date in useRef in useEffect of reservation component',
			...
			case 'update_reservation':
				const updatedState = state.map((reservation) => {
					if (
						...
						reservation.availableSlots > 0
					) {
						return {
							...reservation,
							reservedSlots: reservation.reservedSlots + 1,
							availableSlots:
								reservation.availableSlots - 1 === 0
									? 'No Slot available'
									: reservation.availableSlots - 1,
						};
					}
					return reservation;
				});
	...
	const handleSlots = (dateReserved) => {
		const existingReservation = state.find(
			(reservation) => reservation.date === dateReserved
		);
	...
	const getSubmitedData = (values) => setSubmitData([...values]);
		...
		<div className="reservations">
			{defined && (
				<BookingSlot
					...
					slots={state}
					defaultSlot={[totalSlotCount, reservedSlotCount]}
				/>
			)}
			<BookingForm
				ref={dateRef}
				handleSlots={handleSlots}
				updateRefDateValue={updateRefDateValue}
				slots={state}
